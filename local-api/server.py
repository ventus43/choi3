...
# ── 백오피스 공통 비밀번호 인증 ───────────────────────────────────────────────
# 프론트(auth.js)가 로그인 시 저장해둔 값을 매 요청 헤더(X-Office-Auth)로 실어 보냄.
# 없거나 틀리면 401 — 프론트는 이걸 받으면 저장값을 지우고 로그인 화면으로 돌아감.
...
def check_office_auth():
    if request.method == 'OPTIONS':
        return None
    if request.headers.get('X-Office-Auth', '') != OFFICE_PASSWORD:
